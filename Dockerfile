...
LABEL org.opencontainers.image.title="vortex-ai-gateway" \
      org.opencontainers.image.source="https://github.com/BenvinD/vortex-ai-gateway" \
      org.opencontainers.image.licenses="Apache-2.0"

# No apt layer at all: the HEALTHCHECK below uses the interpreter this image
# already ships rather than installing `curl` for one line. A fixed UID so a
# volume or a pod security policy can name it, and no login shell.
RUN useradd --create-home --uid 10001 --shell /usr/sbin/nologin vortex
